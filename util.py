    ...
    for j in range(len(all_sessions)):
        ...
        if length > 0:  # 去掉训练长度为1的session
            ...
            for i in range(length):
                ...
                data.append(1)
    matrix = csr_matrix((data, indices, indptr), shape=(s_num, n_node))
    return matrix
...
class Data():
    def __init__(self, data, shuffle=False, n_node=None):
        ...
        H_T = data_masks(self.raw, n_node)  # 稀疏矩阵
        ...
        self.adjacency = H_T  #  DHBH_T.tocoo()
        self.n_node = n_node
...
class Get_sessGraph():
    ...
    def get_slice(self, index):
        ...
        for session in inp:
            ...
            mask.append([1]*len(nonzero_elems) + (max_n_node - len(nonzero_elems)) * [0])
            reversed_sess_item.append(list(reversed(session)) + (max_n_node - len(nonzero_elems)) * [0])
        return session_len, items, reversed_sess_item, mask
